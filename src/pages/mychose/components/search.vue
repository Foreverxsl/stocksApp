<template>
    <div class="mint-search" v-bind:class="{searchHeight:visible}">
        <div class="mint-searchbar">
            ...
            <a class="mint-searchbar-cancel" @click="forVisible()" v-show="visible" v-text="cancelText"></a>
        </div>
...
import Vue from 'vue';
    export default {
        name: 'mt-search',
        created: function() {
            var selfStock = localStorage.getItem(this.telphone); //加载自选股数据
            selfStock = JSON.parse(selfStock);
            //判断是否有已添加的自选股，有的话在此基础上操作，没有的话赋值为空
            if (selfStock == null) {
                this.selfStock = []
                localStorage.setItem(this.telphone, JSON.stringify(this.selfStock)); //若是localStorage里面没有selfStock，则将selfStock存入本地，避免其他页面报错 
            } else {
        ...
        watch: {
            currentValue(val) {
                ...
                if (this.currentValue != "") {
                    var stocks = localStorage.allStocks;
                    var dpstocks = localStorage.dpstocks
                    ...
                    /*if (dpstocks == null || dpstocks == "" || dpstocks == "[]" || dpstocks == undefined) {
                        this.getDPStockInfo()
                    }*/
                }
            },
        ...
        methods: {
            ...
            filterResult: function() {
                this.result = []; //清空历史搜索
                this.allStocks = JSON.parse(localStorage.getItem('allStocks'));
                var searchType = 0;
                ...
                for (var i = 0; i < this.allStocks.length; i++) {  
                    ...
                    var abbr = this.allStocks[i].pinyin;
                    this.allStocks[i].stkcode = this.allStocks[i].market + this.allStocks[i].code;
                    switch (searchType) {
                        case 0:
                            ...
                            break;
                        case 1:
                            //匹配abbr,后台返回的数据abbr字段有大写的，也有小写的
                        ...
                        default:
                            //匹配name
                            if ((name.replace(/\s+/g, "")).match((this.currentValue).replace(/\s+/g, ""))) {
            ...
            toDetail: function(item) { //点击搜索结果的每一条跳转至个股详情页
                this.$router.push({
                    path: '/stockDetails/stkCode=' + (item.market+item.code) + '&stkName=' + item.name
                });
            },
            //获取大盘股票基本信息
...
.search .stockIcon {
    ...
    width: 1.1025rem;
    height: 1.125rem;
    background: url("../../../assets/add1.svg");
